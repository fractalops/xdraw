import { arrow, rectangle, text } from "./elements.js";
import { anchor, inset } from "./layout.js";
import { endpointLabelBounds } from "./connector-labels.js";
import {
...
export { fitTextSize } from "./text-metrics.js";
...
export function tone(name, overrides = {}) {
  if (!(name in DEFAULT_TONES)) {
    throw new Error(`unknown tone: ${name}`);
  }
  return { ...DEFAULT_TONES[name], ...overrides };
}

export function heading(id, position, value, options = {}) {
...
export function boundText(id, containerId, bounds, value, options = {}) {
...
export function measureCard(options, width) {
...
export function lane(id, bounds, title, options = {}) {
...
export function card(id, bounds, options) {
  const colors = tone(options.tone ?? "neutral", options.colors);
...
export function connect(id, fromBounds, toBounds, options = {}) {
  ...
  if (options.label) {
    ...
    const intersectsEndpoint = (position) => [fromBounds, toBounds, ...(options.labelObstacles ?? [])].some((bounds) => (
  ...
  const endpointLabel = (position, value, point, nextPoint) => {
    if (!value) return;
  ...
  endpointLabel("end", options.endLabel, path.at(-1), path.at(-2));
  return elements;
}
