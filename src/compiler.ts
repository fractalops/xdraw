...
export function compile(document: CompileInput, options: CompileOptions = {}): Drawing {
  ...
  let scene: SemanticDocument;
  if (document.type === "semantic-document") {
    const validationDiagnostics = validateSemanticDocument(document);
    if (validationDiagnostics.length) throw new DiagnosticError(validationDiagnostics);
    scene = document;
  } else {
    scene = buildSemanticIR(expandDocument(document));
  }
