...
import type { SemanticDocument, NodeStatement, TextStatement } from "../src/contracts/semantic.ts";
import type { DrawingElement, TextElement } from "../src/contracts/render.ts";

function semantic(source: string): SemanticDocument {
  return buildSemanticIR(parseSource(source));
}

function requireNode(document: SemanticDocument, id: string): NodeStatement {
  const statement = document.statements.find((item) => item.type === "node" && item.id === id);
  assert.ok(statement?.type === "node", `missing node ${id}`);
  return statement;
}

function requireFreeText(document: SemanticDocument, id: string): TextStatement {
  const statement = document.statements.find((item) => item.type === "text" && item.id === id);
  assert.ok(statement?.type === "text", `missing text ${id}`);
  return statement;
}

function requireElement(elements: readonly DrawingElement[], id: string): DrawingElement {
  const element = elements.find((item) => item.id === id);
  assert.ok(element, `missing element ${id}`);
  return element;
}

function requireTextElement(elements: readonly DrawingElement[], id: string): TextElement {
  const element = elements.find((item) => item.id === id);
  assert.ok(element?.type === "text", `missing text element ${id}`);
  return element;
}

test("style precedence is theme, semantic kind, constructor defaults, named style, then local properties", () => {
  ...
  const node = requireNode(document, "service");
  node.styleDefaults = { stroke: "#181818", "body-size": 18 };
  ...
  assert.equal(style.bodySize, 18);
  assert.equal(style.fontFamily, 2);
  assert.equal(style.backgroundColor, "#dbeafe");
});

test("visual stroke styles remain style properties rather than connection flags", () => {
  const drawing = compile(parseSource('diagram "Styles" { item: rectangle "Item" { stroke-style = dotted } }')).toJSON();
  assert.equal(requireElement(drawing.elements, "item:frame").strokeStyle, "dotted");
});
...
test("ellipse nodes compile with styled, linked, and locked text", () => {
  ...
  const frame = requireElement(drawing.elements, "target:frame");
  const label = requireTextElement(drawing.elements, "target:title");
...
test("free text supports named wrapping, font, and lock controls", () => {
  ...
  const label = requireTextElement(drawing.elements, "caption_text");
...
test("styles and themes are document-scoped and themes are singular", () => {
  assert.throws(
    ...
    /constructor 'frame' does not accept child kind 'style'/,
  );
  assert.throws(
    () => semantic('diagram "Nested" { box: frame "Box" { base: theme { font-family = normal }; item: rectangle "Item" } }'),
    /constructor 'frame' does not accept child kind 'theme'/,
  );
  assert.throws(
...
test("local free-text properties override named styles and themes", () => {
  ...
  const statement = requireFreeText(document, "label");
...
test("style properties apply only to compatible targets", () => {
  ...
  const node = requireNode(nodeDocument, "item");
  ...
  const label = requireFreeText(textDocument, "label");
...
test("theme properties apply only to compatible targets", () => {
  ...
  assert.equal(resolver.resolveNode(requireNode(document, "item")).titleSize, 19);
  assert.equal(resolver.resolveText(requireFreeText(document, "caption")).fontSize, 24);
});
...
test("explicit neutral palette overrides semantic node defaults", () => {
  ...
  assert.equal(requireElement(result.elements, "implicit:frame").strokeColor, "#7c3aed");
  assert.equal(requireElement(result.elements, "explicit:frame").strokeColor, "#94a3b8");
});
...
test("links require a safe portable protocol", () => {
  for (const link of ["javascript:alert(1)", "file:///tmp/private", "relative/path"]) {
    ...
    const node = requireNode(document, "item");
...
test("style values reject invalid colors and non-finite numbers", () => {
  ...
  const colorNode = requireNode(invalidColor, "item");
  colorNode.attributes.stroke = 12 as never;
...
test("line height is shared by measurement and rendered text", () => {
  ...
  const rich = requireNode(document, "rich");
  ...
  for (const id of ["simple:title", "rich:title", "rich:body", "choice:title"]) {
    assert.equal(requireTextElement(elements, id).lineHeight, 1.8);
  }
});
