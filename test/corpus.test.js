...
for (const entry of entries) {
  ...
  test(`corpus entry ${entry.id} compiles deterministically`, async () => {
    ...
    const first = JSON.stringify(compile(await resolveAssets(parseSource(source), filesystem)).toJSON());
    const second = JSON.stringify(compile(await resolveAssets(parseSource(source), filesystem)).toJSON());
