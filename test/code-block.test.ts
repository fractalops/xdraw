...
import type { DrawingJson, RectangleElement, TextElement } from "../src/contracts/render.ts";
import type { RenderableCodeStatement } from "../src/contracts/semantic.ts";
...
function codeSource(drawing: DrawingJson, id = "example"): TextElement | undefined {
  return drawing.elements.find((element): element is TextElement => (
    element.id === `${id}:source` && element.type === "text"
  ));
}

function codeFrame(drawing: DrawingJson, id = "example"): RectangleElement {
  const element = drawing.elements.find((candidate): candidate is RectangleElement => (
    candidate.id === `${id}:frame` && candidate.type === "rectangle"
  ));
  assert.ok(element, `missing code frame ${id}`);
  return element;
}
...
test("code blocks render one editable source element without wrapping", () => {
  ...
  const language = drawing.elements.find((element) => element.id === "example:language");
  assert.ok(language?.type === "text");
  assert.equal(language.text, "typescript");
  assert.equal(language.textAlign, "right");
});
...
test("mixed rows wrap before shrinking code below its measured width", () => {
  ...
  const container = document.statements[0];
  assert.ok(container && "statements" in container && Array.isArray(container.statements));
  const code = container.statements.find((item) => item.type === "code" && item.id === "examples.sample");
  ...
  assert.ok(frame.width >= codeBlockRequiredWidth(code as RenderableCodeStatement));
...
test("highlighted XDraw source uses the native language vocabulary", async () => {
  ...
  const source = drawing.elements.filter((element): element is TextElement => (
    element.id.startsWith("example:source:") && element.type === "text"
  ));
...
test("code blocks reject invalid content and values", () => {
  ...
  const block = semantic.statements[0];
  assert.ok(block?.type === "code");
  block.lineNumbers = "yes";
  assert.throws(
    () => compile(semantic),
