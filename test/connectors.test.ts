...
test("connections sharing an anchor fan out along that edge", () => {
  const drawing = compile(parse(`diagram "Fan" {
    hub: frame "Hub" {
      arrange column { gap 40 }
      source: rectangle "Source" { size (600, 90) }
      targets: group {
        arrange row { gap 20 }
        a: rectangle "A" { size (160, 80) }
        b: rectangle "B" { size (160, 80) }
        c: rectangle "C" { size (160, 80) }
      }
      source@bottom -> targets.a@top
      source@bottom -> targets.b@top
      source@bottom -> targets.c@top
    }
  }`)).toJSON();
  ...
  // Every arrow leaves the same edge, but not the same point on it.
  const starts = arrows.map((arrow) => arrow.startBinding.fixedPoint);
  assert.ok(starts.every(([, y]) => y === 1), "all three leave the bottom edge");
  assert.equal(new Set(starts.map(([x]) => x)).size, 3, `expected three distinct exits, got ${JSON.stringify(starts)}`);
  assert.ok(starts.every(([x]) => x > 0 && x < 1), "exits stay on the edge");

  // The drawn path must start where the binding says, not all at the centre.
  const startXs = arrows.map((arrow) => Math.round(arrow.x + arrow.points[0][0]));
  assert.equal(new Set(startXs).size, 3, `expected three distinct start points, got ${startXs.join(", ")}`);
});

test("a lone connection still leaves from the middle of its edge", () => {
  const drawing = compile(parse(`diagram "Single" {
    a: rectangle "A" { at (0, 0); size (200, 80) }
    b: rectangle "B" { at (0, 300); size (200, 80) }
    a@bottom -> b@top
  }`)).toJSON();
  const arrow = drawing.elements.find((element) => element.type === "arrow");
  assert.deepEqual(arrow.startBinding.fixedPoint, [0.5, 1]);
});
